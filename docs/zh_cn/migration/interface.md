...
1. [MMEngine](https://github.com/vbti-development/onedl-mmengine)：MMEngine 是 OpenMMLab 2.0 架构的核心，我们将许多与计算机视觉无关的内容从 MMCV 拆分到 MMEngine 中。

2. [MMCV](https://github.com/vbti-development/onedl-mmcv)：OpenMMLab 的计算机视觉包。这不是一个新的依赖，但您需要将其升级到 **2.0.0** 或以上的版本。

3. [MMClassification](https://github.com/vbti-development/onedl-mmpretrain)（可选）：OpenMMLab 的图像分类工具箱和基准。这不是一个新的依赖，但您需要将其升级到 **1.0.0rc6** 版本。

4. [MMDetection](https://github.com/vbti-development/onedl-mmdetection)(可选): OpenMMLab 的目标检测工具箱和基准。这不是一个新的依赖，但您需要将其升级到 **3.0.0** 或以上的版本。
...
事实上，在 OpenMMLab 2.0 中，我们引入了 `Loop` 来控制训练、验证和测试中的行为。`Runner` 的功能也发生了变化。您可以在 [MMMEngine](https://github.com/vbti-development/onedl-mmengine/) 的[执行器教程](https://github.com/vbti-development/onedl-mmengine/blob/main/docs/zh_cn/design/runner.md) 中找到更多的详细信息。
...
新字段 **`default_scope`**：搜索所有注册模块的起点。MMSegmentation 中的 `default_scope` 为 `mmseg`。请参见[注册器教程](https://github.com/vbti-development/onedl-mmengine/blob/main/docs/zh_cn/advanced_tutorials/registry.md)了解更多详情。
